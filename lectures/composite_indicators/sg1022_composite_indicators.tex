...
\frame{
    ...
    \begin{enumerate}
        ...
        \item Multivariate analysis (note: we don't really do much this in this course)
...
\frame{
    ...
    The overall goal is to understand how the variables are related to each other theoretically and empirically.
...
\frame{
    ...
    For example, \emph{life expectancy at birth} is in years ranging from 0 to $>100$ and \emph{GNI per capita} is in US dollars starting from $>300$ and go up to about 100,000.

    \vspace{0.5cm}

    Adding these two variables together would {\large{\textbf{weight GNI more than life expectancy}}}.
}
...
\frame{
    ...
    Note: extreme values can distort the measure and can widen the range of closely spaced values. Could be used for ordinal data.
}

\frame{
    \frametitle{Min-Max example}
    \begin{center}
        \includegraphics[scale=0.2]{figures/min-max_example.png}
    \end{center}
}
...
\frame{
    ...
    \begin{center}
        \includegraphics[scale=0.2]{figures/z-score_example.png}
    \end{center}
}
...
\frame{
    \frametitle{Direction reverse example}
    \begin{center}
        \includegraphics[scale=0.2]{figures/reverse_example.png}
    \end{center}
}
...
\frame{
    ...
    If you simply {\large{\textbf{add all of the variables}}} together, you are implicitly assuming that they have an {\large{\textbf{equal weight}}} of {\large{\textbf{1}}} in the composite index ($CI_{u,t}$). E.g.:
...
\frame{
    ...
    Imagine that you have four component variables that we have normalised: $I_{1}$, $I_{2}$, $I_{3}$ and $I_{4}$.

    \vspace{0.5cm}

    Based on your expert knowledge, you believe that $I_{1}$ is much more important than the other three combined. So you give it a weight of 0.7 and the others 0.1:

    \begin{equation}
        CI_{u,t} = 0.7I_{1u,t} + 0.1I_{2u,t} + 0.1I_{3u,t} + 0.1I_{4u,t}
    \end{equation}
...
\frame{
    ...
    \begin{itemize}
        ...
        \item Error in our construct (e.g. omitting important variables)
