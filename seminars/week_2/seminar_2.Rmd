...
R can load data from many different file formats (e.g. .sav (SPSS), .xlsx (Excel), .dta (Stata), .csv, and so on).
...
# Load ESS5 (UK only) survey data in SPSS format
ess5_uk <- import('ESS5_UKonly.sav')

head(ess5_uk[1:3, 1:6])
```

# Note: Always look at the data you imported to see if it is what you think you imported/if it needs cleaning.

## Rio `export` for saving data

We can save the `ess5_uk` data into another format (.csv "Comma Separated Values").

```{r, eval=FALSE}
export(ess5_uk, 'ESS5_UKonly.csv')
```
...
library(ggplot2); ggplot(survey, aes(Exer, fill = Smoke)) +
    ...
    xlab('Excercise Frequency') + theme_bw()
```
